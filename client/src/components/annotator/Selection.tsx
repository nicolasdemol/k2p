...
interface Props {
  ...
  pdfContext: CanvasRenderingContext2D | null;
}
    ...
    if (selectionRef && entity) {
      ...
      switch (entity.entityType) {
        case "NER": {
          ...
          const markToAdd: AnnotationParams = buildNerAnnotation(
            pageNumber,
            entity,
            selectionChildren,
            coordsToUse
          ) || { nerAnnotation: { textIds: [], tokens: [] } }; // Fournir une valeur par défaut si buildNerAnnotation renvoie undefined

          if (
            markToAdd.nerAnnotation &&
            markToAdd.nerAnnotation.textIds.length
          ) {
            if (keyPressed) {
